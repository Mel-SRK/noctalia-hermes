...
busy 信号在 CLI 进程活跃期间会一直保持，不受时间限制。如果 CLI 异常退出（未触发 `on_session_end`），busy 信号会在 60 秒后自动过期回退到进程检测状态，防止状态卡死。attention 信号会保持到 Hermes 发出 `post_approval_response`，手动 attention 则保持到 `hermes-attention clear`。
